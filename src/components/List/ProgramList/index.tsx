...
const ProgramListWithFiltersAndPagination: React.FC<ListWithFiltersAndPaginationProps> = ({
  useDataHook,
  renderItem,
  filterConfig,
  ...
  const renderFilters = () => (
    <div className="min-h-[500px] rounded-lg border bg-white p-6 shadow-sm">
      ...
      {/* Filters Accordion */}
      <Accordion defaultValue={['price-range', 'course_category', 'is_fire_sale', 'program_type', 'date_begin']} type="multiple" className="w-full">
...
function ProgramListWithFiltersAndPaginationPage(props: any) {
  return (
    <Suspense fallback={<p>Loading...</p>}>
      <ProgramListWithFiltersAndPagination {...props} />
    </Suspense>
  );
}

export default ProgramListWithFiltersAndPaginationPage;
